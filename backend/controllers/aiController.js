...
import { chatCompletion, SYSTEM_PROMPTS, parseJSON } from "../utils/aiService.js";
  ...
  try {
    ...
    const context = habits
      .map((h) => {
        ...
        return `Habit: ${h.name} (${h.category}): ${hLogs.length}/30 in last 30 days, by weekday [Sun,Mon,Tue,Wed,Thu,Fri,Sat]: [${byDow.join(",")}]`;
      })
      .join("\n");
